...
- (void)didMoveToWindow{
	...
	if(! theWheel){
		...
		if(self.overlyImage){
			overlay = [[UIImageView alloc] initWithImage:self.overlyImage];
			overlay.userInteractionEnabled = NO;
			overlay.image = self.overlyImage;
			[self addSubview:overlay];
		}
		
		if(self.maskImage){
			CGSize maskSz = self.maskImage.size;
			CALayer* maskLayer = [CALayer layer];
			maskLayer.frame = CGRectMake(0, 0, maskSz.width, maskSz.height);
